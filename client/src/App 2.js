...
function App() {
  ...
  const runTest = async () => {
    ...
    if (invalidQuestions.length > 0) {
      alert(`The following questions have probabilities that don't sum to 100%: ${invalidQuestions.map(q => q.questionName).join(', ')}`);
      return;
    }
    
    setIsRunning(true);
    setLogs(['Starting survey test run...']);
    
    try {
      const response = await fetch('/api/runSurvey', {
        method: 'POST',
        headers: {
